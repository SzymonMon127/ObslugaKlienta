package com.example.obslugaklienta.ui.NotFinishedComplaint;
...
import com.example.obslugaklienta.ObjectsAndAdapters.CaptionedImagesAdapterComplaints;
...
public class NotFinishedComplaintFragment extends Fragment implements ValueEventListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        View layout = inflater.inflate(R.layout.fragment_not_finished_complaint, container, false);

        mainActivityRecyclerView = layout.findViewById(R.id.Fragment_not_finished_complaint_recycler);
    ...
    public void onPause() {
        super.onPause();
        firebaseDatabase.getReference().child("NotFinishedComplaint").orderByChild("hour").removeEventListener(this);
    ...
    public void onResume() {
        super.onResume();
        fail = false;
        firebaseDatabase.getReference().child("NotFinishedComplaint").orderByChild("hour").addValueEventListener(this);
    }
    ...
    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
        ...
        ArrayList<String> cityList9 = new ArrayList<>();
        for (DataSnapshot postSnapshot : dataSnapshot.getChildren()) {
            Order newOrder = postSnapshot.getValue(Order.class);
            assert newOrder != null;
            cityList9.add(newOrder.complaint);
        }
        ...
        final String[] complaintTable = new String[size];
        for (int i = 0; i < size; i++) {
            complaintTable[i] = cityList9.get(i);
        }
        ...
        CaptionedImagesAdapterComplaints
                adapter = new CaptionedImagesAdapterComplaints(hour, order, adress, phoneNumber, price, payment, complaintTable, pointsStringTAble);
        ...
        adapter.setListener(position -> {
            ...
            complaint = complaintTable[position];
    ...
    public void showAlertDialogButtonClicked() {
        ...
        builder.setMessage("Na pewno chcesz przenieść zamówienie z godziny:"+" " +hour1 +" do zrealizowanych reklamacji? ");
        ...
        builder.setPositiveButton("TAK, CHCĘ", (dialog, which) -> {
            try {
                ...
                firebaseDatabase.getReference("Complaint").child(id1).setValue(newOrder1);
            ...
            catch (Exception e)
            {
                ...
                if (!fail)
                {
                    firebaseDatabase.getReference("NotFinishedComplaint").child(id1).removeValue();
        ...
        builder.setNeutralButton("COFAM DO ZREALIZOWANYCH", (dialog, which) -> {
            try {
                fail = false;
