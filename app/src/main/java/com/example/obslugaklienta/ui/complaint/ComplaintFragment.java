package com.example.obslugaklienta.ui.complaint;
...
public class ComplaintFragment extends Fragment implements ValueEventListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        mainActivityRecyclerView = layout.findViewById(R.id.Fragment_complaint_recycler);
    ...
    public void onPause() {
        super.onPause();
        firebaseDatabase.getReference().child("Complaint").orderByChild("hour").removeEventListener(this);
    ...
    public void onResume() {
        super.onResume();
        firebaseDatabase.getReference().child("Complaint").orderByChild("hour").addValueEventListener(this);
    }
    ...
    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
        ...
        for (DataSnapshot postSnapshot : dataSnapshot.getChildren()){
            Order newOrder = postSnapshot.getValue(Order.class);
            assert newOrder != null;
            cityList8.add(newOrder.complaint);
        }
        ...
        for (DataSnapshot postSnapshot : dataSnapshot.getChildren()){
            Order newOrder = postSnapshot.getValue(Order.class);
            assert newOrder != null;
            cityList9.add(newOrder.userNameClient);
        }
        ...
        for(int i = 0; i < size; i++)
        {
            complaintTable[i] = cityList8.get(i);
        }

        final String[] userNameClientTable = new String[size];
        for(int i = 0; i < size; i++)
        {
            userNameClientTable[i] = cityList9.get(i);
        }
        ...
        CaptionedImagesAdapterComplaints adapter = new CaptionedImagesAdapterComplaints(hour, order, adress, phoneNumber, price, payment, complaintTable, pointsStringTAble);
    ...
    public void showAlertDialogButtonClicked() {
        ...
        builder.setTitle("Cofanie reklamaji");
        builder.setMessage("Na pewno chcesz wycofać reklamację z godziny:"+" " +hour1 +" i przenieść z powrotem do zrealizowanych?");
        ...
        builder.setPositiveButton("TAK, CHCĘ", (dialog, which) -> {
            ...
            catch (Exception e)
            {
                ...
                if (!fail)
                {
                    firebaseDatabase.getReference("Complaint").child(id1).removeValue();
